...
class Dataset(object):
	...
	def next(self):
		...
		if self.mode == "single":
			...
			# assume nullsym's index is 0 (to reduce calculation)
			x = np.zeros((x_len, len(x_proto)), dtype='int64') # (sent_len, batch_size)
			y = np.zeros((len(y_proto), y_len), dtype='int64') # (batch_size, sent_len)
			...
			for i in range(self.config['batch_size']):
				...
				y[i,:len(y_proto[i])] = y_proto[i] # batch major
...
def sentence_i2w(sentence, i2w_list, nullsym):
	return ' '.join([ i2w_list[sentence[i]] for i in range(sentence.shape[0]) if sentence[i]!=nullsym ])
